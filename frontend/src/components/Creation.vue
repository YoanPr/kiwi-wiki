<template>
    ...
    <div class="bg-indigo-100">
        <div class="container grid place-items-center max-w-5xl mx-auto gap-y-1 bg-indigo-100">
            <p v-if="isNew" class="text-center mt-4">Oups ! Aucun article n'est disponible à cette URL, mais vous pouvez dès maintenant le créer en utilisant le formulaire ci-dessous ! </p>
            <h3 class="text-2xl mt-2">{{ isNew ? "Ajouter un " : "Modification d'un "}} article</h3>
            <h4 class="mb-3"> Adresse de l'article : {{urlComplete }}</h4>
            <p class="mb-3"> Titre de l'article : 
            <input class="mb-3 ml-3" id="titreArticle" type="text" placeholder="Titre article" v-bind:value="title"><br>
            </p> <Edition :paramContenuArticle=contenu />
